document.addEventListener('DOMContentLoaded', () => {
  ...
  async function summarizeWebPage() {
    ...
    if (pageData.error || !pageData.text) {
      ...
      resultContainer.innerHTML = `<p class="message">获取网页内容失败: ${pageData.error || '未知错误'}</p>`;
...
- 文本长度：${pageData.text.length}字符${pageData.truncated ? '（已截断）' : ''}`;
    ...
    sidePanelDiv.className = 'action-button';
    sidePanelDiv.style.marginTop = '25px';
    sidePanelDiv.innerHTML = '<button id="open-sidepanel" class="open-sidepanel">在侧边栏中打开</button>';
    resultContainer.appendChild(sidePanelDiv);
  ...
  function showHistory() {
    ...
    // 日志显示当前历史记录数量
    console.log(`准备显示${historyRecords.length}条历史记录`);
  ...
  function updateUI() {
    ...
    // 清除结果区域并显示提示信息
    resultContainer.innerHTML = '<p class="message">请选择或输入要' + (activeMode === 'translate' ? '翻译' : '总结') + '的文本</p>';
  ...
  async function processText() {
    ...
    if (!text) {
      console.log('无文本可处理');
      resultContainer.innerHTML = '<p class="message">请先选择或输入要处理的文本</p>';
...
- 文本长度：${text.length}字符`;
  ...
  // 初始化: 从存储中恢复设置
  chrome.storage.local.get(['activeMode', 'inputMode', 'targetLanguage', 'summaryLength', 'summaryLanguage', 'inputText'], (result) => {
    console.log('从存储恢复设置:', result);
